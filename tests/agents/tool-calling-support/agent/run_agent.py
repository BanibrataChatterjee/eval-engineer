...
ROOT_DIR = EXAMPLE_DIR.parents[2]
sys.path.insert(0, str(EXAMPLE_DIR))
load_dotenv(ROOT_DIR / ".env")

from agent.tool_policy import select_tools_for_context  # noqa: E402
...
def build_user_message(user_message: str, account_context: dict) -> str:
...
async def run_test_case(user_message: str, account_context: dict):
    ...
    try:
        ...
        tools = select_tools_for_context(load_tools(), user_message, account_context)
        formatted_message = build_user_message(user_message, account_context)

        response = await client.messages.create(
            model=os.getenv("EVAL_ENGINEER_AGENT_MODEL", DEFAULT_MODEL),
            max_tokens=4000,
            temperature=0,

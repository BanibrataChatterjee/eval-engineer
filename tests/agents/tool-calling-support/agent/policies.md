...
For tool selection, wire transfer refund handling maps to `escalate_to_billing`
with reason `wire_transfer_issue` unless an explicit EEA/UK compliance rule,
legal request, or data-protection request applies. Do not use an EU-specific
compliance reason for non-EEA/UK regional cooling-period cases.
...
When cancellation and refund cannot be completed automatically because the
payment method requires manual wire handling, create the required billing
escalation rather than issuing a separate cancellation tool call first.
